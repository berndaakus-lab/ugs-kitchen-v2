      ...
      <div className="min-h-screen bg-brand-cream">
        ...
        <header className="sticky top-0 z-30 bg-brand-cream/95 backdrop-blur border-b border-brand-muted">
          <div className="max-w-lg mx-auto flex items-center gap-3 px-4 h-[60px]">
            <Link
              href="/"
              className="w-9 h-9 flex items-center justify-center rounded-full bg-brand-muted"
            >
              <ArrowLeft size={18} />
            </Link>
            <div>
          ...
          {orders.length === 0 ? (
            ...
            orders.map(order => {
              ...
              return (
                <div
                  key={order.id}
                  className="bg-white rounded-2xl shadow-sm overflow-hidden border border-brand-muted"
                >
                  {/* Order header */}
                    ...
                    <button
                      ...
                      className={`w-full flex items-center justify-center gap-2 rounded-2xl py-3 font-extrabold text-sm transition-colors
                        ${reordered === order.id
                          ...
                          : 'bg-brand-muted text-brand-dark active:bg-gray-200'
                        }`}
                    >
                      <RotateCcw size={15} />
