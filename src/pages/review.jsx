import Head from 'next/head'
import Link from 'next/link'
import { useState, useEffect } from 'react'
import { useRouter } from 'next/router'
import { Star, Send, ArrowLeft, CheckCircle2, XCircle, Clock } from 'lucide-react'

function StarInput({ value, onChange }) {
  const [hovered, setHovered] = useState(0)
  const labels = ['', 'Poor', 'Fair', 'Good', 'Great', 'Excellent!']
  return (
    <div className="flex flex-col items-center gap-2">
      <div className="flex gap-2">
        {[1, 2, 3, 4, 5].map(star => (
          <button
            key={star}
            type="button"
            onClick={() => onChange(star)}
            onMouseEnter={() => setHovered(star)}
            onMouseLeave={() => setHovered(0)}
            className="transition-transform active:scale-90"
          >
            <Star
              size={44}
              fill={(hovered || value) >= star ? '#F38F1D' : 'none'}
              stroke={(hovered || value) >= star ? '#F38F1D' : '#D1D5DB'}
              strokeWidth={1.5}
            />
          </button>
        ))}
      </div>
      <span className="text-sm font-bold text-brand-orange h-5">
        {labels[hovered || value] || ''}
      </span>
    </div>
  )
}

// ── State screens ─────────────────────────────────────────────

function Screen({ icon, iconBg, title, body, action }) {
  return (
    <div className="min-h-screen bg-brand-cream flex items-center justify-center px-6">
      <div className="bg-white rounded-3xl p-8 w-full max-w-sm text-center shadow-xl animate-fade-in">
        <div className={`w-20 h-20 rounded-full ${iconBg} flex items-center justify-center mx-auto mb-5`}>
          {icon}
        </div>
        <h1 className="text-xl font-extrabold text-brand-dark mb-2">{title}</h1>
        <p className="text-gray-500 text-sm mb-6">{body}</p>
        {action}
      </div>
    </div>
  )
}

// ── Main page ─────────────────────────────────────────────────

export default function ReviewPage() {
  const router = useRouter()
  const { token } = router.query

  const [state,    setState]    = useState('loading') // loading | valid | invalid | expired | used | done
  const [reason,   setReason]   = useState('')
  const [name,     setName]     = useState('')
  const [rating,   setRating]   = useState(0)
  const [comment,  setComment]  = useState('')
  const [submitting, setSubmitting] = useState(false)
  const [error,    setError]    = useState('')

  useEffect(() => {
    if (!router.isReady) return
    if (!token) { setState('invalid'); return }

    fetch(`/api/review/validate?token=${token}`)
      .then(r => r.json())
      .then(data => {
        if (data.valid) {
          setName(data.customerName ?? '')
          setState('valid')
        } else {
          setReason(data.reason ?? 'invalid')
          setState(data.reason === 'expired' ? 'expired' : data.reason === 'already_used' ? 'used' : 'invalid')
        }
      })
      .catch(() => setState('invalid'))
  }, [router.isReady, token])

  async function handleSubmit(e) {
    e.preventDefault()
    setError('')
    if (rating === 0) return setError('Please choose a star rating.')

    setSubmitting(true)
    const res = await fetch('/api/review/submit', {
      method:  'POST',
      headers: { 'Content-Type': 'application/json' },
      body:    JSON.stringify({ token, rating, comment }),
    })
    const data = await res.json().catch(() => ({}))
    setSubmitting(false)

    if (!res.ok) { setError(data.message || 'Could not submit. Please try again.'); return }
    setState('done')
  }

  // ── Non-form states ──────────────────────────────────────────

  if (state === 'loading') {
    return (
      <div className="min-h-screen bg-brand-cream flex items-center justify-center">
        <p className="text-gray-400 font-semibold text-sm animate-pulse">Verifying your review link…</p>
      </div>
    )
  }

  if (state === 'done') {
    return (
      <>
        <Head><title>Thank You · UGs Kitchen</title></Head>
        <Screen
          iconBg="bg-green-100"
          icon={<CheckCircle2 size={44} className="text-green-500" />}
          title="Thank You! 🙏"
          body="Your review has been saved. We truly appreciate your feedback — it helps us serve you better!"
          action={
            <Link href="/" className="block w-full bg-brand-brown text-white font-extrabold rounded-2xl py-4 text-base active:bg-brand-dark transition-colors">
              Order Again 🍽️
            </Link>
          }
        />
      </>
    )
  }

  if (state === 'used') {
    return (
      <>
        <Head><title>Already Reviewed · UGs Kitchen</title></Head>
        <Screen
          iconBg="bg-yellow-100"
          icon={<CheckCircle2 size={44} className="text-yellow-500" />}
          title="Already Reviewed"
          body="You've already submitted a review for this order. Thank you for your feedback!"
          action={
            <Link href="/" className="block w-full bg-brand-brown text-white font-extrabold rounded-2xl py-4 text-base active:bg-brand-dark transition-colors">
              Order Again 🍽️
            </Link>
          }
        />
      </>
    )
  }

  if (state === 'expired') {
    return (
      <>
        <Head><title>Link Expired · UGs Kitchen</title></Head>
        <Screen
          iconBg="bg-gray-100"
          icon={<Clock size={44} className="text-gray-400" />}
          title="Review Link Expired"
          body="This review link is more than 7 days old and has expired. Review links are sent by SMS when your order is delivered."
          action={
            <Link href="/" className="block w-full bg-brand-brown text-white font-extrabold rounded-2xl py-4 text-base active:bg-brand-dark transition-colors">
              Go to UGs Kitchen
            </Link>
          }
        />
      </>
    )
  }

  if (state === 'invalid') {
    return (
      <>
        <Head><title>Invalid Link · UGs Kitchen</title></Head>
        <Screen
          iconBg="bg-red-100"
          icon={<XCircle size={44} className="text-red-400" />}
          title="Invalid Review Link"
          body="This review link is not valid. You'll receive a unique link by SMS after your order is delivered."
          action={
            <Link href="/" className="block w-full bg-brand-brown text-white font-extrabold rounded-2xl py-4 text-base active:bg-brand-dark transition-colors">
              Go to UGs Kitchen
            </Link>
          }
        />
      </>
    )
  }

  // ── Review form ──────────────────────────────────────────────

  return (
    <>
      <Head>
        <title>Leave a Review · UGs Kitchen</title>
        <meta name="description" content="How was your UGs Kitchen experience?" />
        <meta name="viewport" content="width=device-width, initial-scale=1" />
      </Head>

      <div className="min-h-screen bg-brand-cream">
        <header className="sticky top-0 z-10 bg-brand-cream/95 backdrop-blur border-b border-brand-muted">
          <div className="max-w-lg mx-auto flex items-center gap-3 px-4 h-[60px]">
            <Link href="/" className="w-9 h-9 rounded-xl bg-brand-muted flex items-center justify-center">
              <ArrowLeft size={18} />
            </Link>
            <div>
              <p className="font-extrabold text-brand-dark text-base leading-tight">UGs Kitchen</p>
              <p className="text-[10px] text-gray-400">Share your experience</p>
            </div>
          </div>
        </header>

        <div className="max-w-lg mx-auto px-4 py-8">
          <div className="text-center mb-8">
            <img src="/logo-ugs.jpeg" alt="UGs Kitchen" className="w-16 h-16 object-contain mx-auto mb-4" />
            <h1 className="text-2xl font-extrabold text-brand-dark mb-1">How was your meal? 😋</h1>
            {name && <p className="text-gray-500 text-sm">Hi {name}! We'd love to hear from you.</p>}
          </div>

          <form onSubmit={handleSubmit} className="bg-white rounded-3xl p-6 shadow-sm border border-brand-muted space-y-5">

            <div className="flex flex-col items-center py-2">
              <label className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-3">Your Rating</label>
              <StarInput value={rating} onChange={setRating} />
            </div>

            <div className="border-t border-gray-100" />

            <div>
              <label className="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-1">
                Comment <span className="normal-case font-normal text-gray-400">(optional)</span>
              </label>
              <textarea
                value={comment}
                onChange={e => setComment(e.target.value)}
                placeholder="Tell us about your experience…"
                rows={4}
                className="w-full border-2 border-gray-200 rounded-xl px-4 py-3 text-base font-semibold outline-none focus:border-brand-orange transition-colors resize-none"
              />
            </div>

            {error && (
              <p className="text-red-500 text-sm font-semibold bg-red-50 rounded-xl px-4 py-3">{error}</p>
            )}

            <button
              type="submit"
              disabled={submitting}
              className="w-full flex items-center justify-center gap-2 bg-brand-brown text-white font-extrabold rounded-2xl py-4 text-base active:bg-brand-dark transition-colors disabled:opacity-60"
            >
              <Send size={18} />
              {submitting ? 'Submitting…' : 'Submit Review'}
            </button>

            <p className="text-[11px] text-gray-400 text-center">
              This link is unique to your order and can only be used once.
            </p>
          </form>
        </div>
      </div>
    </>
  )
}
